...
string pitch_eq = "1*sin(2*10*pi*t+0)"; // default pitch
string roll_eq = "1*sin(2*10*pi*t+0)";  // default roll
float pi = 3.141592653;                 // needs to be of type float, and in the math lib its of type double
float time2run = 2.4;                   // Hard coded default time to run, 2.4 sec
float sample_time = .004;               // sample time of 250 HZ
...
int actualposition_p;                   // Actual position for the pitch, single number
int actualposition_r;                   // Actual position for the roll, single number after compensation
int p_multip = 1433;                    // conversion factor, for the pitch, from output amp to rotational amplitude in increments
int r_multip = 682;                     // conversion factor, for the roll, from output amp to rotational amplitude in increments
void* g_pKeyHandle = 0;                 // maxon
...
float multi_fun(string before, string before_type, string after, string after_type, float t);   // function to help make multiplying multiple things ie 1*5*2
void  LogError(string functionName, int p_lResult, unsigned int p_ulErrorCode);                 // Maxon
...
int ProfilePositionMode(HANDLE p_DeviceHandle, HANDLE p_DeviceHandle_r, unsigned short p_usNodeId, unsigned short p_usNodeId_r, unsigned int & p_rlErrorCode){
	...
	if((VCS_ActivatePositionMode(p_DeviceHandle, p_usNodeId, &p_rlErrorCode) == 0)||(VCS_ActivatePositionMode(p_DeviceHandle_r, p_usNodeId_r, &p_rlErrorCode) == 0)){
		...
		//VCS_GetPositionIs(p_DeviceHandle, p_usNodeId, &actualposition_p, &p_rlErrorCode);         // these three lines of code are to output the pitch position
		...
		//VCS_GetPositionIs(p_DeviceHandle_r, p_usNodeId_r, &actualposition_r, &p_rlErrorCode);     // these three lines of code are to output the roll position
...
void UserInput(){
    ...
    printf("\n\n\n\n\n\n\nFor entering desired angular position equations follow the rules outlined below.\n1. If one element is 0 then enter 0\n2. No coefficient greater then 1, full range of motion is an amplitude of 1\n3. NO division, only decimals ie 1/2 = 0.5\n4. No variable other then t, time\n5. Multiplication requires asterisk, *");
    printf("\n--------------------------------------------------------------------------------------------------------------------------------------");
    printf("\nFormat: X(t) = A*f(w*t+p)\nExample: X(t) = 0.78*sin(2*pi*25*t+90)");
  ...
  //  printf("\nFormat: t = t\nExample 1: X(t) = 5\nExample 2: X(t) = 1.234\n ");
   ...
   // EnterTime();          // no user control over time to run in this code version.
    DoubleCheck();
...
void EnterPitch(){
    ...
    // if user entered y, yes it is correct, move on. if user entered n, no, then call enter pitch again
    if (correct == "y"){
...
//    cout << "\nFor the desired time to run you entered: " << time2run << " seconds";
    ...
    // if user entered y, yes it is correct, move on. if user entered n, no, then call enter roll again
    if (correct == "y"){
        return;
    }
    else {
